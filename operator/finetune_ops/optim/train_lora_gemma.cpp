...
// Lightweight npy float32 loader for alignment/debug use only
...
struct CliOptions {
    ...
    // JSONL masked data (optional; JSONL mode is enabled when any is provided)
    ...
    // Optional: in alignment mode override LoRA A weights using PT dump
    std::string align_pt_weights_dir;
    // Optional: in alignment mode run finite-difference numeric check on layer0 attn_out_raw
    bool align_numeric_attn = false;
    float align_numeric_eps = 1e-3f;
    int align_numeric_count = 4;  // take first few elements per target
    std::string align_numeric_targets = "attn_out_raw_l0";
    std::string target_mode = "full";
    std::string lora_targets_override;  // comma-separated; overrides target_mode when non-empty
    int epochs = 1;
    int max_steps = -1;
    int seq_len = 256;
    int batch = 4;
    ...
    std::string loss_reduction = "mean";  // can switch to sum_debug for alignment debugging
...
int main(int argc, char** argv) {
    try {
        ...
        // 2. Tokenizer (optional: placeholder when using pre-tokenized data)
        ...
        if (!cli.lora_targets_override.empty()) {
            // Parse comma-separated targets to override defaults; useful for alignment/debugging single or partial LoRA modules.
        ...
        if (!cli.align_dump_dir.empty()) {
            lora_spec.dropout = 0.0f;  // disable dropout in alignment mode
        }
        GemmaLoraInjector injector;
        injector.inject(model, lora_spec);
        injector.print_info();

        // Optional: in alignment mode override initial LoRA A weights with PT dump (to verify random stream differences)
        ...
        if (!cli.jsonl_train.empty() || !cli.jsonl_valid.empty() || !cli.jsonl_test.empty()) {
            ...
            // JSONL mode does not require tokenizer eos/pad
            data_cfg.eos_id = -1;
            data_cfg.pad_id = 0;
        ...
        // Alignment mode: run a small forward pass and dump intermediate activations/loss
        if (!cli.align_dump_dir.empty()) {
            ...
            logits->set_requires_grad(true);  // allow dumping dlogits during alignment
            logits->retain_grad();
            ...
            save_npy(cli.align_dump_dir + "/per_token_nll.npy",
                     ...
                     DumpDType::kFloat32);
            // Use lm_cross_entropy output (with reduction) directly for loss_scalar to stay aligned with PT baseline.
            ...
            if (cli.align_dump_grads) {
                ...
                if (!cli.align_disable_debug) {
                    ...
                    // Export analytical dMat/dQ/dK/dV for each layer into grads/analytical_*
                    {
                        ...
                        for (int layer : target_layers) {
                            ...
                            // dV_heads = probs^T @ G_context (restore by head)
                            if (ctx && ctx->grad()) {
                    ...
                    // Numeric gradient check: finite difference on selected targets
                    if (cli.align_numeric_attn) {
                        auto debug_map_base = model.debug_tensors();  // Gradients from baseline forward
                ...
                if (cli.align_do_step) {
                    // Single-step AdamW (wd=0) applied only to LoRA parameters
                    AdamConfig acfg;
