以下脚本封装了从数据准备到开始 LoRA 微调与评测的最小可运行流程（基于 Gemma 3 270M 权重）。
...
- 预训练权重通过 `MFT_MODEL_ROOT`、`GEMMA_270M_MODEL_DIR` 或当前目录 `pretrained/` fallback 提供

使用顺序：
1) 生成 JSONL（masked 标签，仅答案位置参与损失）
   bash run_prepare_data.sh
   输出目录：仓库根目录 `runs/mmlu_jsonl_gemma270m_s128/`
